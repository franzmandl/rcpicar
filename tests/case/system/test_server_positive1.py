...
def test() -> None:
    ...
    throttles = server_ctx.server.car_arguments.throttle.throttles.get()
    ...
    with server_ctx.server.service_manager.get():
        ...
        mock_clock.sleep_till(
            lambda: server_ctx.server.latency_service.get().received_messages in mock_clock.waiting_queues)
        mock_clock.tick(9)
        ...
        mock_clock.notify_till(lambda: server_ctx.server.latency_service.get().received_messages.empty())
        assert server_ctx.server.car_service.get().throttle_service.current_throttle.get() == throttles[0]
        ctx.udp_client_to_server_queue.put(
